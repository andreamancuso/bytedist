...
- Stage 20 Vite integration is complete:
  - `bytedist/vite` exposes a build-only Vite plugin;
  - the plugin can emit a `.bytedist` asset or embed the generated payload into HTML;
...
Status: Complete for a minimal build-only Vite plugin, virtual metadata module, docs, and a framework-neutral single-file example. Dev-server serving and broader bundler integration remain future work.
...
- Complete. `docs/vite.md` documents the plugin scope, build-only behavior, emit vs embed modes, virtual metadata module, single-file tradeoffs, and no-DRM/no-secrets limits.
...
- Complete. `bytedist/vite` exports `bytedistPlugin`, which applies only during Vite builds. It packs an input directory with existing Node helpers, emits a `.bytedist` asset when configured, embeds generated payload bytes into HTML when configured, can embed optional WASM bytes, and exposes `virtual:bytedist/payload` build metadata. Determinism follows the current sorted `packDirectory` behavior; broader deterministic-build controls remain Stage 21.
...
- Complete. `examples/vite-single-file/` is a framework-neutral Vite example that embeds a generated ByteDist payload into built HTML and reads it with `bytedist/browser`.
